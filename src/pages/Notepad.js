	...
	async function Save() {
		...
		if (title.length > 30) {
			...
			if (toedit === true) {
				if (authenticated === true) {
					try {
						const _id = posts[idx]._id
						const { data } = await api.put('/notes/edit', {
							id: _id,
							title: title,
							note: note,
						})
						const userid = localStorage.getItem('IdUser')
						const tempPosts = { _id, userid, title, note }
				...
				if (authenticated === true) {
					...
					try {
						...
						const _id = data._id
						const tempPosts = { userid, _id, title, note }
	...
	async function Remove(index) {
		if (authenticated === true) {
			try {
				...
				const { data } = await api.delete(`/notes/del/${_id}`)
				const tempNotes = [...posts]
					...
					<Textarea>
						<form>
							...
							<textarea
								...
								onChange={handleInputChange}
							/>
							<button type="button" id="btn" onClick={Save}>
								{' '}
								Salvar{' '}
							</button>
						</form>
					...
					<SavedNotes>
						{posts.map((post, index) => (
							<div key={index}>
								...
								<ContainerNotes>{post.note}</ContainerNotes>
							</div>
						))}
					</SavedNotes>
				</Notes>
